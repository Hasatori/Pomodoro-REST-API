...
public interface DirectMessageRepository extends JpaRepository<DirectMessage, Integer> {
    ...
    @Modifying
    @Query(value = "SELECT * FROM DIRECT_MESSAGE dm LEFT JOIN REACTION r ON (dm.ID=r.MESSAGE ) WHERE RECIPIENT=:userId and r.READ_TIMESTAMP IS NULL", nativeQuery = true)
    List<DirectMessage> findAllUnreadMessages(@Param("userId") Integer userId);

    @Modifying
    @Query(value = "SELECT * FROM DIRECT_MESSAGE dm LEFT JOIN REACTION r ON (dm.ID=r.MESSAGE ) WHERE RECIPIENT=:recipientId AND AUTHOR_ID=:authorId and r.READ_TIMESTAMP IS NULL", nativeQuery = true)
    List<DirectMessage> findAllUnreadMessagesFromUser(@Param("authorId") Integer authorId,@Param("recipientId") Integer recipientId);
